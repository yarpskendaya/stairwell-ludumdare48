...
TouchInput.prototype.moveSensitivity = 1;
TouchInput.prototype.zoomSensitivity = 1;
...
TouchInput.prototype.onTouchStartEndCancel = function(event) {
    ...
    // we store the current touch position
...
//The following code is disabled to not allow the camera's pivot point to change (no panning)
/*
...
TouchInput.prototype.onTouchMove = function(event) {
    ...
    if (touches.length == 1) {
        var touch = touches[0];
        var move = {};
        move.x = (touch.x - this.lastTouchPoint.x) * -this.moveSensitivity;
        move.y = (touch.y - this.lastTouchPoint.y) * -this.moveSensitivity;
        ...
        var zoom = diffInPinchDistance * -this.zoomSensitivity;
        this.app.fire("input-zoom", zoom);
